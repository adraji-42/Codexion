/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   init.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: adraji <adraji@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/21 10:55:32 by adraji            #+#    #+#             */
/*   Updated: 2026/04/21 10:55:32 by adraji           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "codexion.h"

static t_bool	destroy_dongles(t_sim *sim, int i, t_bool mtx_init)
{
	if (mtx_init)
		pthread_mutex_destroy(&sim->dongles[i].mtx);
	while (i-- > 0)
	{
		pthread_mutex_destroy(&sim->dongles[i].mtx);
		pthread_cond_destroy(&sim->dongles[i].cv);
	}
	free(sim->dongles);
	sim->dongles = NULL;
	free(sim->coders);
	sim->coders = NULL;
	return (FALSE);
}

static t_bool	destroy_coders(t_sim *sim, int i)
{
	while (i-- > 0)
		pthread_mutex_destroy(&sim->coders[i].mtx);
	return (destroy_dongles(sim, sim->n_coders, FALSE));
}

static t_bool	init_dongles(t_sim *sim)
{
	int	i;

	i = -1;
	while (++i < sim->n_coders)
	{
		if (pthread_mutex_init(&sim->dongles[i].mtx, NULL))
			return (destroy_dongles(sim, i, FALSE));
		if (pthread_cond_init(&sim->dongles[i].cv, NULL))
			return (destroy_dongles(sim, i, TRUE));
		sim->dongles[i].id = i;
		sim->dongles[i].is_held = FALSE;
		sim->dongles[i].available_at = 0;
		sim->dongles[i].sim = sim;
		sim->dongles[i].heap.size = 0;
		sim->dongles[i].heap.sim = sim;
	}
	return (TRUE);
}

static t_bool	init_coders(t_sim *sim)
{
	int	i;

	i = -1;
	while (++i < sim->n_coders)
	{
		if (pthread_mutex_init(&sim->coders[i].mtx, NULL))
			return (destroy_coders(sim, i));
		sim->coders[i].sim = sim;
		sim->coders[i].id = i + 1;
		sim->coders[i].compile_count = 0;
		sim->coders[i].last_compile_start = 0;
		if (!(sim->coders[i].id % 2))
		{
			sim->coders[i].first = &sim->dongles[i];
			sim->coders[i].second = &sim->dongles[(i + 1) % sim->n_coders];
		}
		else
		{
			sim->coders[i].first = &sim->dongles[(i + 1) % sim->n_coders];
			sim->coders[i].second = &sim->dongles[i];
		}
	}
	return (TRUE);
}

t_bool	init_sim(t_sim *sim)
{
	sim->dongles = malloc(sizeof(t_dongle) * sim->n_coders);
	if (!sim->dongles)
		return (FALSE);
	sim->coders = malloc(sizeof(t_coder) * sim->n_coders);
	if (!sim->coders)
		return (destroy_dongles(sim, 0, FALSE));
	if (pthread_mutex_init(&sim->print_mtx, NULL))
		return (destroy_dongles(sim, 0, FALSE));
	if (pthread_mutex_init(&sim->state_mtx, NULL))
	{
		pthread_mutex_destroy(&sim->print_mtx);
		return (destroy_dongles(sim, 0, FALSE));
	}
	if (!init_dongles(sim) || !init_coders(sim))
	{
		pthread_mutex_destroy(&sim->print_mtx);
		pthread_mutex_destroy(&sim->state_mtx);
		return (FALSE);
	}
	return (TRUE);
}
